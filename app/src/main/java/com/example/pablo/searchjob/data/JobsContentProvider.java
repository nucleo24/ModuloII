...
public class JobsContentProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        int match = uriMatcher.match(uri);
        Log.d("CONSULTA:::",String.valueOf(match));
        switch (match) {
            case JOBS:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case JOBS:
                retValue = dbHelper.getWritableDatabase().insert(JobPostDbContract.JobEntry.TABLE_NAME,null, values);
                if (retValue != -1) {
                    returnUri = JobPostDbContract.JobEntry.CONTENT_URI.buildUpon().appendPath(Long.toString(retValue)).build();
                } else {
                    throw new SQLException("Error inserting Job");
                }
                break;
            case CONTACTS:
                retValue = dbHelper.getWritableDatabase().insert(JobPostDbContract.ContactEntry.TABLE_NAME,null, values);
                if (retValue != -1) {
                ...
                break;
            default:
                throw new UnsupportedOperationException("Not yet implemented1111");
        }
